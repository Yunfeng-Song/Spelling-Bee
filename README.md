...
* [`src/cljs/spelling_bee/`](src/cljs/spelling_bee/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/spelling_bee/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `spelling-bee` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/spelling_bee/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns spelling-bee.example
  (:require [spelling-bee.config :as config])
